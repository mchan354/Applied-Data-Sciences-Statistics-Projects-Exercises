...
We merged the master and Economic data set and we deleted all of our data that had missing values for our analysis
```{r}
...
We used the spread function to show the males and females along with their corresponding suicide number and population 
```{r,warning=F}
...
We set a user defined function to calculate the mode of the suicide numbers.
...
A scatterplot is generated showing the relationship between population and suicide numbers. We can conclude that when the population is between 0 and 2x10^7 , most of the number of sucides are between 0 and 5000. The greatest number of suicides occur with a population of 2x10^7 with a number of 20000 associated with some countries.
...
```
### Scatter plots showing the relationship between the factor levels sex between population and sucicides per population in 1000, gdp for year vs suicides per population in 1000, NV.SRV.TOTL.ZS vs sucides per population in 1000, NV.IND.TOTL.ZS  vs per population in 1000,  NV.IND.MANF.ZS vs sucicides per population in 1000, and  NV.AGR.TOTL.ZS vs sucicides per population in 1000. 
